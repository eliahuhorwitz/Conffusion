...
def make_dataset_extracted_bounds(dir):
    if os.path.isfile(dir):
        files = [i for i in np.genfromtxt(dir, dtype=np.str, encoding='utf-8')]
    else:
        files = []
        assert os.path.isdir(dir), '%s is not a valid directory' % dir
        for root, _, fnames in sorted(os.walk(dir)):
...
class BoundsInpaintDataset(data.Dataset):
    def __init__(self, data_root, mask_config={}, data_len=-1, image_size=[256, 256], loader=pil_loader, sampled_bounds_path=None):
        imgs = make_dataset(data_root)
        lower_bounds = make_dataset_extracted_bounds(f"{sampled_bounds_path}/lower_bounds/")
        upper_bounds = make_dataset_extracted_bounds(f"{sampled_bounds_path}/upper_bounds/")
        sampled_masks = make_dataset_extracted_bounds(f"{sampled_bounds_path}/masks/")
        masked_samples = make_dataset_extracted_bounds(f"{sampled_bounds_path}/masked_samples/")
